...
exports.inviteUser = onRequest(
  { region: "europe-west1", cors: ["http://localhost:3000", "https://aio-arbetsorder.web.app"] }, // Lägg till din produktions-URL för CORS
  async (req, res) => {
    if (req.method === "OPTIONS") {
      res.status(204).send("");
      return;
    ...
    //   const adminUserDoc = await db.collection('users').doc(decodedToken.uid).get(); // Eller din admin-samling
    ...
    //   res.status(401).send({ error: "Unauthorized: Invalid token or role check failed." });
    //   return;
    // }

    const { email, role = "user" } = req.body.data || req.body; // Hantera både anrop från callable och direkt HTTP
    ...
    try {
      ...
      try {
        ...
        if (error.code === "auth/user-not-found") {
          console.log(`User ${email} not found in Firebase Auth. Creating new user.`);
          userRecord = await admin.auth().createUser({
            email: email,
            emailVerified: false, // Användaren måste verifiera sin e-post
            // Du kan sätta ett slumpmässigt lösenord här om du vill,
            // men Firebase skickar vanligtvis ett e-postmeddelande för kontoinställning.
            // password: "someRandomPassword", // Överväg säkerhetsimplikationer
            displayName: email.split("@")[0], // Standardvisningsnamn
          });
          console.log(`Successfully created new user in Firebase Auth: ${userRecord.uid} for email: ${email}`);
          // Firebase Auth skickar vanligtvis ett e-postmeddelande för att slutföra kontoinställningen.
          // Om du vill skicka ett anpassat e-postmeddelande kan du göra det här.
        } else {
          throw error; // Andra Auth-fel
        }
      }

      // Kontrollera om användaren (baserat på UID) redan finns i schedulableUsers
      const schedulableUsersRef = collection(db, "schedulableUsers");
      const q = query(schedulableUsersRef, where("uid", "==", userRecord.uid));
      const querySnapshot = await getDocs(q);

      if (!querySnapshot.empty) {
        console.log(`User with UID ${userRecord.uid} already exists in schedulableUsers.`);
        // Optionally update their role if it's different, or just inform the client.
        const existingDoc = querySnapshot.docs[0];
        if (existingDoc.data().role !== role) {
          await existingDoc.ref.update({ role: role, email: userRecord.email }); // Uppdatera roll och e-post om den ändrats
           console.log(`Updated role for existing schedulableUser ${userRecord.uid} to ${role}.`);
        }
        res.status(200).send({
          message: `User ${email} already exists and is configured. Role updated if different.`,
          uid: userRecord.uid,
          isNew: false,
        });
        return;
      ...
      const newUserInData = {
        ...
        name: userRecord.displayName || email.split("@")[0], // Använd displayName från Auth om det finns
        role: role,
        createdAt: Timestamp.now(),
      };

      const newUserDocRef = await addDoc(schedulableUsersRef, newUserInData);
      console.log(`User ${email} (UID: ${userRecord.uid}) added to schedulableUsers with ID: ${newUserDocRef.id}`);

      res.status(201).send({
        message: `User ${email} has been ${userExistsInAuth ? "configured" : "invited"}. They need to complete their account setup if new.`,
        uid: userRecord.uid,
      ...
      if (error.code === "auth/email-already-exists" && !error.uid) {
         // Detta kan hända om e-posten finns men inte kunde hämtas av någon anledning.
         res.status(409).send({ error: `An account with email ${email} already exists but could not be fully processed. Please check Firebase Console.` });
      } else if (error.code === "auth/email-already-exists") {
         res.status(409).send({ error: `An account with email ${email} already exists. UID: ${error.uid}` });
      } else {
        res.status(500).send({ error: "Failed to invite user. " + error.message });
      }
    }
  }
);
